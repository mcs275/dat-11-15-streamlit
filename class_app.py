...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  5 18:51:05 2022
...
st.title("My First Dashboard YAY!!")

url = r"https://raw.githubusercontent.com/JonathanBechtel/dat-11-15/main/ClassMaterial/Unit1/data/master.csv"

num_rows = st.sidebar.number_input('Select Number of Rows to Load', min_value = 10000, 
                                   max_value = 50000, step = 1000)
...
def load_data(num_rows):
    df = pd.read_csv(url, parse_dates= ['visit_date'], nrows=1000)
    return df
...
if section == 'Data Explorer':
    ...
    id_val= st.sidebar.selectbox("Choose Restaurant ID",
                                  df['id'].unique().tolist())
    yesterday = st.sidebar.number_input("How many visitors yesterday",
                                        min_value = 0, max_value = 100, step = 1, value =20)
    day_of_the_week = st.sidebar.selectbox("Day of Week", df['day_of_week'].unique().tolist())
    ...
    prediction = model.predit(sample)[0]
